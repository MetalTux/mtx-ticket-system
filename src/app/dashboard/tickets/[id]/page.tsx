...
export default async function TicketDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // 1. Consulta extendida con las nuevas relaciones de tablas maestras
  const [masters, ticket] = await Promise.all([
    getTicketMasters() ,
    db.ticket.findUnique({
      where: { id },
      include: {
        status: true,     // Nueva tabla
        priority: true,   // Nueva tabla
        category: true,   // Nueva tabla
        creator: true,
        createdBy: true,
        client: true,
        assignedTo: true,
        history: {
          include: { 
            user: true,
            status: true,   // Para ver qué estado se asignó en ese momento
            priority: true,
            category: true
          },
          orderBy: { createdAt: "desc" } // Normalmente el historial más reciente va arriba
        }
      },
    })
  ]);

  // Verificación de existencia y pertenencia (Seguridad Multitenant)
  if (!ticket || ticket.providerId !== session.user.providerId) notFound();

  // 2. Usuarios de soporte para el panel de gestión
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 border-b border-slate-100 dark:border-slate-800 pb-4 lg:pb-6">
        ...
        <div className="flex items-center gap-2 w-full lg:w-auto">
           <span 
             ...
             style={{ 
               backgroundColor: `${ticket.status.color}15`, // 15 es opacidad en HEX
        ...
        <div className="lg:col-span-8 space-y-2 lg:space-y-8 order-2 lg:order-1">
          <div className="card-module bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 shadow-sm !p-4 lg:!p-8">
            <div className="space-y-3 mb-6 lg:mb-8">
              ...
              {/* Tags dinámicos de Categoría y Prioridad */}
          ...
          {/* PANEL DE GESTIÓN */}
          {/* Usamos systemKey para lógica de cierre en lugar de strings fijos */}
          {ticket.status.systemKey !== "CLOSED" && (
            <div className="p-1 lg:p-6 pt-1 lg:pt-8">
              <TicketManagementPanel 
                ...
                statuses={masters.statuses!} // <-- AGREGAR
                priorities={masters.priorities!} // <-- AGREGAR
