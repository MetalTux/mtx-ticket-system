...
import { X, Paperclip, Mail, Settings2, AlertCircle } from "lucide-react";
...
// Definimos un tipo extendido para el Ticket que incluya sus relaciones
...
interface TicketManagementProps {
  ...
  supportUsers: User[];
  statuses: TicketStatus[];     // Nuevas props dinámicas
  priorities: TicketPriority[]; // Nuevas props dinámicas
  categories: TicketCategory[]; // Nuevas props dinámicas
  userRole: string;
}
...
export default function TicketManagementPanel({ 
  ...
  // Implementamos useFormState para capturar errores de validación del "Escudo"
  ...
  return (
    <div className="card-module border-t-4 border-t-brand-600 bg-white dark:bg-slate-900 shadow-xl border-slate-200 dark:border-slate-800 overflow-hidden">
      <div className="p-1 lg:p-8">
        ...
        {state?.message && (
          <div className="mb-6 flex items-center gap-3 p-4 bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/20 rounded-xl text-red-600 dark:text-red-400 text-xs font-bold">
            <AlertCircle size={18} />
            {state.message}
          </div>
        )}
        ...
        >
          ...
          {/* Si no es soporte, enviamos los valores actuales para no alterarlos */}
          {!isSupport && (
            <>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-1 lg:gap-4">
              ...
              <div className="space-y-1 lg:space-y-2">
                ...
                <select 
                  ...
                  className="form-input text-[11px] lg:text-xs py-2 dark:bg-slate-800 dark:border-slate-700 w-full"
                >
